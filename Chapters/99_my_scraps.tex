...
% dlGloss.tex. I have some information in that document
...
%************************************************************
\section{Verbatim}
% To eliminate a page-break for a Verbatim section, use samepage:
\begin{Verbatim}[commandchars=~\[\], samepage=true]
whatever
\end{Verbatim}

%I also defined the following two special verbatim environments
\begin{binDisp} % to display binary math problems
whatever
\end{binDisp}

\begin{lineDisp} % displays line numbers on the verbatim text
whatever
\end{lineDisp}
...
%************************************************************
\section{acronyms}
% ***Note: I now use the glossary function, not acronyms***
% Define a new acronym in "Contents.tex"
% The definition takes the form: \acro{POS}{Product of Sums}
% Enter them in alphabetic order for the Table of Acronymns 
...
\lstinputlisting[firstline=7]{ckt/cl.fn.v}
...
\begin{figure}[H]
	...
	%********************************************************************  
	% Text in top-Left cell
	\node[] at (0.50,3.35) { $ \mathsf{ \mathbf{B} } $ }; % B
	\node[] at (1.15,4.00) { $ \mathsf{ \mathbf{A} } $ }; % A
	...
	% Populate the minterms
	\foreach \loc/\txt in { 
		(2.75,1.75)/{00} , (4.25,1.75)/{02} , 
		(2.75,0.25)/{01} , (4.25,0.25)/{03} 
	}
	\draw \loc node{ \color{blue!90!black} \small { $\txt$ }};
	...
	\draw
	% Finish drawing the grid
	[step=1.5cm,black,thin] (0,0) grid (4.5,4.5) % The Grid
	(0.0,4.5) -- (1.5,3.0) % Diagonal in the top left cell
	(1.5,3.05) -- (4.5,3.05) % Double line under top header row
	(1.45,0.0) -- (1.45,3.0) % Double line on left of header column one
	;    
	\end{tikzpicture}
	\caption{KMap 2X2}
	\label{kmap:08_02}
